...
3. **Default stack** -- Next.js 14+ (App Router), Tailwind CSS, Supabase (auth + DB + storage), deployed on Vercel. Only deviate when the problem demands it (e.g., mobile = React Native, real-time = WebSockets).
4. **Revenue from day one** -- Prefer ideas with an obvious monetization path. Free tools are a hobby, not a product.
5. **Solo-dev friendly** -- No ideas requiring ops teams, content pipelines, or partnerships to function.
