...
rank               Manually re-rank all scouted/ranked ideas
...
1. **ideas** -- Router runs a 3-step pipeline: Researcher gathers market signals (saved to `research.json`), Scout generates 5 ideas grounded in research, Ranker scores them. Ideas scoring >= 5.0 become `ranked`; below 5.0 become `filtered`. User sees only passing ideas.
...
3. **rank** -- Router sends scouted/ranked ideas to Ranker. Ranker returns scores. Router merges `ranking` into each idea in `pipeline.json`.
...
|--------|-------|---------|
| `scouted` | Scout | Idea generated by Scout |
| `ranked` | Ranker | Scored >= 5.0, ready for spec |
| `filtered` | Ranker | Scored below 5.0, hidden from user |
| `superseded` | - | Replaced by a refined version |
| `specced` | PM | PM has written a build spec |
| `designed` | Designer | Designer has created a design system |
| `scaffolded` | Builder | Scaffold pushed to GitHub |
| `developed` | Developer | Developer has implemented all code |
| `qa_pass` | QA | QA validated, ready to deploy |
| `qa_fail` | QA | QA found issues, needs fixing |
| `deployed` | Deployer | Live on Vercel |
