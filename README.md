...
git clone https://github.com/eitandooreckaloni/openclaw-set-up.git
cd openclaw-set-up
...
gh variable set DOMAIN --body "eitan-openclaw.duckdns.org" --repo eitandooreckaloni/openclaw-set-up
...
```
You (Telegram) ──► @EitanOpenclawAgentBot
                        │
                        ▼
              Telegram Bot API
                        │
...
# On OLD server:
./scripts/backup.sh
scp openclaw-backup-*.tar.gz root@NEW_SERVER_IP:~/openclaw-set-up/
...
- Message `@EitanOpenclawAgentBot` on Telegram with an app idea
